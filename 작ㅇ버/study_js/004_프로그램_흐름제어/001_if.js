    ...
    // AND(&&) 연산은 하나라도 거짓이 포함되면 결과가 거짓이므로 아래의 조건은 실행x
    ...
    // 조건이 거짓이므로 출력되지 않음. a = true, !a = false
    if (!a) {
    ...
    // true || false는 참이지만 그 결과를 부정했으므로 결과는 거짓이 된다.
    if (!(x6 || y6)) {
