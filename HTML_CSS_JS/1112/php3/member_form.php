...
<?php
session_start();
    if (isset($_SESSION["userid"]))/*로그인이 되어있는지 확인*/ $userid = $_SESSION["userid"];
    else $userid = "";/* 로그인이 안되어있으면 빈값 */
?>

<?php // 로그인이 되어있을 때 회원가입창으로 가는걸 막음
	session_start();
		...
		<fieldset class="register">
			...
			<ul>
				<li>
					...
					<input type="text" id="new_id" name="id" size="20" minlength="3" maxlength="15" required> <small style="color:red;"> 3~15자리 이내의 영문과 숫자</small> <!-- 경고 -->
                    <input type="button" value="중복 확인" onclick="idcheck()">
                    <p id="result"></p> <!-- result -->
				</li>
				<li>
